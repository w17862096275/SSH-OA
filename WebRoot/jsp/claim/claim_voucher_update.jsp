	...
	<head>
		...
		<script type="text/javascript">
			...
			$("#AddRow").click(function(){		
				...
				var desc = $("#desc").val();
				tr.find("td").get(0).innerHTML="<input  name=sets["+j+"].item id=item"+j+" type=hidden value="+item+" />"+item;
				tr.find("td").get(1).innerHTML="<input  name=sets["+j+"].account id=account"+j+"  type=hidden value="+account+" />"+account;
				tr.find("td").get(2).innerHTML="<input  name=sets["+j+"].des  id=desc"+j+" type=hidden value="+desc+" />"+desc;		
				...
				$(".notice").text("*");
				$("#rowNumber").attr("value",i);
		
			});	
			
		});
		var i=parseInt(<s:property value="claimVoucher.bizClaimVoucherDetails.size"/>);
		
		var rowNumber=parseInt(<s:property value="claimVoucher.bizClaimVoucherDetails.size"/>);
		...
		function delRow(id){	
			...
			var rowNumber=$("#rowNumber").val();
				...
				$("#item"+s).attr("name","sets["+(s-1)+"].item");
				$("#item"+s).attr("id","item"+(s-1));
				$("#account"+s).attr("name","sets["+(s-1)+"].account");
				$("#account"+s).attr("id","account"+(s-1));
				$("#desc"+s).attr("name","sets["+(s-1)+"].desc");
		...
		<div class="pages">
			...
			<input type="hidden" id="claimId" name="claimVoucher.id" value="<s:property value="claimVoucher.id"/>"/>
				...
				<table id="myTable" width="90%" border="0" cellspacing="0" cellpadding="0" class="addform-base">
					...
					<tr>
						<td>
							<s:hidden id="id%{#s.index}" name="sets[%{#s.index}].id" value="%{#claimDetail.id}"/>
							<s:hidden id="item%{#s.index}" name="sets[%{#s.index}].item" value="%{#claimDetail.item}"/>
							<s:property value="#claimDetail.item"/></td>
						<td>
							<s:hidden id="account%{#s.index}" name="sets[%{#s.index}].account" value="%{#claimDetail.account}"/>
							<s:property value="#claimDetail.account"/></td>
						<td>
							<s:hidden id="desc%{#s.index}" name="sets[%{#s.index}].des" value="%{#claimDetail.des}"/>
